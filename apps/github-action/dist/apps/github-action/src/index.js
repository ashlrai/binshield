...
async function main() {
    ...
    if (config.registerDependencies && config.apiKey) {
        try {
            const registered = await client.registerDependencies(targets.map((target) => ({ ecosystem: "npm", packageName: target.name, version: target.version })));
            core.info(`Registered ${registered} dependencies with BinShield for proactive alerting.`);
        }
        catch (error) {
            core.warning(`BinShield dependency registration failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
